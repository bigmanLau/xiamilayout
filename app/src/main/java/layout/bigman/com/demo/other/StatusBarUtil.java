...
//        window.setStatusBarColor(Color.TRANSPARENT);
    ...
    /*解决化为tab栏叠重问题*/
    public static void huaWeiNavigationBar(Activity baseActivity) {
        if (!checkDeviceHasNavigationBar(baseActivity)) {
            return;
        }
        mChildOfContent = baseActivity.findViewById(android.R.id.content);
    ...
    /*显示与隐藏全屏 true 显示 */
    public static void showFullScreen(Activity activity, boolean isFull) {
        if (activity == null) {
            return;
        }
        if (isFull) {
